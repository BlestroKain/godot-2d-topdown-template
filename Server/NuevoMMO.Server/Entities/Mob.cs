...
/// <summary>
/// Estado runtime de una criatura hostil/combatible. La Definition sigue siendo la fuente
/// de configuración; aquí solo viven vitales, amenaza, aggro y timers mutables.
/// </summary>
public sealed class Mob : LivingEntity
{
    ...
    public Mob(EntityId id, MobDefinition definition, MapInstanceId mapInstance, Vector2Data position)
        : base(
            ...
            mapInstance,
            position,
            RequireDefinition(definition).VisualKey,
            definition.Name,
    ...
    public void ResetAggro(bool clearThreat = true)
    {
        AggroOrigin = null;
        LeaveCombat();
        if (clearThreat) threat.Clear();
    }
    ...
    public EntityId? HighestThreat(Func<EntityId, bool>? predicate = null)
    {
        ...
        return candidates.MaxBy(static pair => pair.Value).Key;
    }
    ...
    public bool IsOutsideResetRadius()
    {
        ...
        return delta.LengthSquared > Behavior.ResetRadius * Behavior.ResetRadius;
    }

    public bool ShouldFlee()
        => IsAlive && Behavior.FleeHealthPercentage > 0 && HealthPercent <= Behavior.FleeHealthPercentage;

    public bool CanBasicAttack(long nowMilliseconds)
        => IsAlive && nowMilliseconds >= LastBasicAttackMilliseconds + Math.Max(0, Combat.AttackIntervalMilliseconds);

    public void MarkBasicAttack(long nowMilliseconds)
    {
        if (nowMilliseconds < 0) throw new ArgumentOutOfRangeException(nameof(nowMilliseconds));
        LastBasicAttackMilliseconds = nowMilliseconds;
    }

    public bool CanUseTechnique(long nowMilliseconds)
        => IsAlive && nowMilliseconds >= LastTechniqueMilliseconds + Math.Max(0, Combat.TechniqueIntervalMilliseconds);

    public void MarkTechnique(long nowMilliseconds)
    {
        if (nowMilliseconds < 0) throw new ArgumentOutOfRangeException(nameof(nowMilliseconds));
        LastTechniqueMilliseconds = nowMilliseconds;
    }
    ...
    private static MobDefinition RequireDefinition(MobDefinition? definition)
    ...
    private static int ReadMaxVital(CreatureCombatDefinition combat, VitalId vital, int fallback, int minimum)
    {
        ...
        if (!float.IsFinite(value) || value < minimum || value > int.MaxValue)
            throw new ArgumentOutOfRangeException(nameof(combat), $"Vital {vital} inválido.");

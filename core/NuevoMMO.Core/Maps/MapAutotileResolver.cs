...
public sealed class MapAutotileResolver
{
    ...
    public MapAutotileRenderData Resolve(Vector2IntData cell)
    {
        ...
        var basePixel = new Vector2IntData(tile.AtlasCell.X * tileSize.X, tile.AtlasCell.Y * tileSize.Y);
    ...
    public IReadOnlyCollection<Vector2IntData> AffectedCells(Vector2IntData changedCell)
    {
        ...
        var cliffAffected = affected.Any(cell => tiles[cell].Autotile == MapAutotileMode.Cliff) ||
                            (tiles.TryGetValue(changedCell, out var changed) && changed.Autotile == MapAutotileMode.Cliff);
        if (cliffAffected)
        {
            foreach (var tile in tiles.Values)
    ...
    private bool CheckMatch(Vector2IntData sourceCell, Vector2IntData targetCell)
    {
        ...
        return source.TilesetKey == target.TilesetKey && source.AtlasCell == target.AtlasCell;
    }

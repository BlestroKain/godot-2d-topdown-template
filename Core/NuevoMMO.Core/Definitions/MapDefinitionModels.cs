...
public sealed record MapTilePlacementDefinition
{
    ...
    /// <summary>
    /// Origen en píxeles del tile en el atlas. Alternative desplaza filas extra de TileSize
    /// para variantes empaquetadas debajo de la celda base.
    /// </summary>
    public Vector2IntData AtlasPixelOrigin(Vector2IntData tileSize)
        => new(AtlasCell.X * tileSize.X, (AtlasCell.Y + Alternative) * tileSize.Y);
}

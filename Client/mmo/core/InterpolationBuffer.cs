...
{
    private readonly List<(double Time, WorldPosition Position)> samples = [];
    public void Add(double time, WorldPosition position)
    {
    ...
    public WorldPosition Sample(double time)
    {
        ...
        for (var index = 1; index < samples.Count; index++)
        {
            ...
            if (time <= after.Time) return WorldPosition.Lerp(before.Position, after.Position,
                (float)((time - before.Time) / (after.Time - before.Time)));
        }
        return samples[^1].Position; // No unbounded extrapolation on a stalled connection.
    }
}
